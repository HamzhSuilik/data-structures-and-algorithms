...
Write a function named sortByChildren that sorts the characters below by the number of children in 
each house (fewest to most). If a house has the same number of children, sort alphabetically by house name.
...
const updateNumbers = (obj) => {
  ...
  keys.forEach((element,index)=>{
    array.push(`${keys[index]}: ${values[index]}`);
  })
  return array;
};
...
const houseSize = (arr) => {
  ...
  arr.forEach(element=>{
    let number=0;
    if(element.spouse){number++}
    if(element.name){number++}
    number=number+element.children.length;

    sizes.push(`{ house: ${element.house} , members: ${number}}`);
  })
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(element=>{
    ...
    deceasedSpouses.forEach(person=>{
      if(element.name==person){number--}
      if(element.spouse==person){number--}
      element.children.forEach(child=>{
        if(child==person){number--}
      })
    })
