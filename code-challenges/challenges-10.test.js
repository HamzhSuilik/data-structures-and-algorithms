...
const getNames = (arr) => {
  ...
  arr.map(item=>{
    const leter = item.name.match(/[A-Za-z]/g);
    const leter2=[];
    for(let i=leter.length;i>-1;i--){
      leter2.push(leter[i]);
    };
    result.push(leter2.join(''));
  });
  return result;
};
...
let findMaleAndFemale = (data) => {
  ...
  data.map(ele=>{
    if(ele.gender.includes('female') || ele.gender.includes('male') ){
      result.push(ele.name);
    }
  })

  const str = result.join(' and ')
  return str;
};
...
Write a function named findShortest that, given the Star Wars data from Challenge 6, uses any combination 
of filter, map and reduce to return the name of the shortest character.
------------------------------------------------------------------------------------------------ */
...
let findShortest = (data) => {
  ...
  data.map(ele=>{
    if(ele.name.length<=long ){
      name=ele.name;
      long=ele.name.length;
    }
  })
  return name;
};
