...
const isNum = (input) => {
  input=input+'';
  const pattern = /[0-9]/g;
  const check = input.match(pattern);
  if (check==null){
    return false;
  }
  return true;
...
Write a function named isCapitalized that takes in a string. This function should use a regular expression pattern
 to match all words that begin with a capital letter. It should only match words, not punctuation.
...
const citiesAtoJ = (arr) => {
  const pattern1 = /\b[A-z].*?\b/g;
  const pattern2 = /\b[A-J].*?\b/g;
  if(arr.length===0){
    return [];
  }
  const arr2=[];
  arr.forEach(element => {
    let new_array =element.match(pattern1);
    new_array = new_array[0].match(pattern2);
    if(new_array!=null){
      arr2.push(new_array[0]);
    }  
  });
  return arr2;
};
...
const matchMonth = (input) => {
  const pattern1 = /\boct/g;
  const pattern2 = /\bOct/g;
  const pattern3 = /\bOctober/g;
  const pattern4 = /\boctober/g;
  const str1 = input.match(pattern1);
  const str2 = input.match(pattern2);
  const str3 = input.match(pattern3);
  const str4 = input.match(pattern4);
...
For example, if given the string "Hello, and have a wonderful day!", the word "Hello, "
 would not be returned because it is immediately followed by a comma. The word "day!" 
 would not be returned because it is immediately followed by an exclamation point.
...
const noPunctuation = str => {
  str=str+'';
  const check1=str.split(' ');
  const arr=[];
  const pattern = /[A-Za-z]/g;

  check1.forEach(element=>{
    const text1=element;
    const array = element.match(pattern);
    if (text1.length===array.length){
      arr.push(text1);
    }
  });
...
Write a function named hangman which uses the replace method to remove all of the vowels
 (a, e, i, o, u) from the hangman string, regardless of capitalization, and replace them with an underscore.
...
let hangman = (str) => {
  const pat =/e|o|a|u|i/g;
  let result = str.replace(pat, "_");
  return result;
};
...
Write a function named findShells that takes in the string below and uses a regular expression pattern 
to find all instances of the following words: "sells", "shells", "seashells".
...
const findShells = (str) => {
  const arr=str.split(' ');
  const pat =/(sells)|(shells)|(seashells)/g;
  const new_arr=[];
  arr.forEach(ele=>{
    const check = ele.match(pat);
    if(check){
      new_arr.push(check[0]); 
    }
  });
  return new_arr;
};
