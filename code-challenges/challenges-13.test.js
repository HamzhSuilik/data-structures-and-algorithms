...
Write a function named findHappiness that takes in an array of strings and returns an array containing
 only the strings from the input array that contain ":)".
...
const standardizePhoneNumbers = (arr) => {
  let result =[];
  const pat =/[0-9]/g;
  const result_arr=''.match(pat);
  arr.forEach(ele=>{
    const all_numbers = ele.match(pat);
    const str = all_numbers.join('');
    result.push(str)
  })
...
const onlyOddChars = (str) => {
  ...
  let result ='';
  for(let i=1;i<str_length;i=i+2){
...
Write a function named findAnything that takes in an array of strings, along with a
 target string. Return an array containing only those strings from the original array that contain the target string.
------------------------------------------------------------------------------------------------ */

const findAnything = (arr, target) => {
  const result = [];
  arr.forEach(element=>{
    if(element.includes(target)){result.push(element)}
  })
  return result;
};
...
const findEvery = (arr, target) => {
  let check = true;
  arr.forEach(element=>{
    check = check &&  element.includes(target);
  })
  return check;
};
...
We've been testing a new course enrollment system, and we think we have the bugs worked out, but in the meantime, Brook enrolled himself 
in a bunch of different classes to test if it was working.

Write a function named unenrollBrook that takes in a two-dimensional array, where each array represents one course's roster 
and is an array of strings of the names of the people in that course.
...
const sortByDay = (arr) => {
  ...
  daysOfWeek.forEach(element=>{
    let array=[];
    arr.forEach(tasks=>{
      if(tasks.includes(element)){array.push(element)}
    })
    matrix.push(array);
  })
  return matrix;
