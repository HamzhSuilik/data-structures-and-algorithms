...
const sortStarWarsCharacters = (starWarsArr) => {
  ...
  const arr = starWarsArr.sort(sort1);
  return arr;
}
...
Write a function named removeThree that takes an index and an array. The function should removes 
...
const listFoods = (recipe) => {
  ...
  array.forEach(element=>{
    let free = element.replace(/[1-9]/g,'');
    free=free.replace(' medium-sized ','');
    free=free.replace(' pounds ','');
    free=free.replace(' pound ','');
    free=free.replace(' gallons ','');
    free=free.replace(' cups ','');
    result.push(free);
  });
