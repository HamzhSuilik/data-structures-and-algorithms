...
function transformToLis(obj){
  ...
  const result=[];
  keys.forEach((a,i)=>{
    result.push(`<li>${keys[i]}: ${values[i]}</li>`);
  })
  return result;
}
...
const addValues = (arr) => {
  let arr2=arr;
  arr2.push(0);
  const reducer = (accumulator, a) => accumulator + a;
...
const addPurchases = (arr) => {
  ...
  function reducer (accumulator, a){
    return a.purchasePrice+accumulator.purchasePrice;
  }
...
const countNumberOfElements = (arr) => {
  ...
  const reducer = (accumulator, a) => accumulator + 1;
...
const reversedString = (str) => {
  ...
  function reducer (accumulator, a ,i){
    accumulator.push(arr[arr.length-1-i])
    return accumulator;
  }
