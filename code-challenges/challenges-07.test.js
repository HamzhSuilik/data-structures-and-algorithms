...
const replaceZeros = (string) => {
  const str =string.replace((/[0]/g),'zero');
  return str;
}
...
Write a function named forEachTwoToThe that produces the same output as your 
forLoopTwoToThe function from challenge 1, but uses forEach instead of a for loop.
------------------------------------------------------------------------------------------------ */
...
function from challenge 1 and your forEachTwoToThe function from challenge 2, but uses map instead of a for loop or forEach.
------------------------------------------------------------------------------------------------ */
...
Write a function named charCode that, given an array of letters as an input, uses map to return
 a new array where each element is the result of the `charCodeAt` method on the original array element.
...
const extractAbilities = (arr) => {
  ...
  arr.map(element=>{
    result.push(element.ability.name);
  })
...
const extractStats = (arr) => {
  ...
  arr.map(element=>{
    result.push(`{ name: ${element.stat.name}, total: ${element.effort + element+baseStat} }`);
  })
